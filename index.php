...
<div class="panel panel-default" ><!-- start recent div -->
            ...
            <div class="panel-body" style="color:#767676; background-color: #7272e2">
            <center><h2>RECENT SCORES</h2></center>
            </div>
				...
				if ($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						 ...
						 echo'<b class="text-uppercase" style="margin-left: 10px; margin-right: 10px">';
                           ...
                           echo $row['singledouble'].'/ '.$row['agegroup'].'/ '.$row['menwomen'].'/ '.$row['matchcat'];
...
<div class="panel panel-default" style=" overflow-y: auto; height: 600px !important" >
	...
	<div class="panel-body" style="color:#767676;">
		...
		<?php
 include 'include/dbhandler.php';
      $sql = "SELECT tableno, matchcat, singledouble, agegroup, menwomen, country1, country2, round1, round2, round3, round4, round5, matchwinner FROM matchresult";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        <td style='text-align:left; padding-bottom:15px;'><b><font size='2'> ".$row["singledouble"]." | ".$row["agegroup"]." | ".$row["menwomen"]." ( ".$row["matchcat"].")</font></b></td>
